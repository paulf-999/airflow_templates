...
with DAG(
    ...
    # note: the jinja below needs to be escaped (i.e., double up every curly bracket)
    # as an f-string is as used an input for the bash_command
    bash_operator_eg = BashOperator(
        task_id="dbt_conn_test",
        bash_command=f"set -e; cd {dbt_project_dir}; dbt {{{{ dag_run.conf['src_tbl'] }}}} --profiles-dir profiles",
        dag=dag,
        retries=0
    )
    ...
    bash_operator_eg = BashOperator(
        task_id="dbt_conn_test",
        bash_command="set -e;dbt debug --project-dir /opt/airflow/dags/dbt/dbt_ofw --profiles-dir /opt/airflow/dags/dbt/dbt_ofw/profiles",
        dag=dag,
        retries=0
    )
    """
