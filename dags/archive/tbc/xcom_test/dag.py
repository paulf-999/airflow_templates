...
with DAG(dag_id=dag_name, doc_md=doc_md, default_args=default_args, schedule_interval=None, tags=["template"]) as dag:
    ...
    t3 = BashOperator(task_id="t3", bash_command="set -e; cd ~/ && ls -la ; echo $?", dag=dag)
    t4 = BashOperator(task_id="t4", bash_command='set -e; echo "{{ ti.xcom_pull(task_ids="t3") }}"', dag=dag)
    t3 >> t4
