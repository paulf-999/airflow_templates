...
with DAG(dag_id=os.path.basename(__file__).replace(".py", ""), default_args=default_args, schedule_interval=None, tags=["python", "template"]) as dag:
    gen_op_tsk = PythonOperator(task_id="gen_op_eg", python_callable=gen_op, provide_context=True)

    read_op_in_sub_tsk = PythonOperator(task_id="read_op_in_sub_task", python_callable=read_op, provide_context=True)
