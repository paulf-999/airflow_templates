...
def read_op(**kwargs):
    ...
    for i in ip:
        logger.info(f"i = {i}")
        if i == 1:
            raise AirflowException("Example task error. i = 1")
...
default_args = {"owner": "airflow", "depends_on_past": False, "email_on_failure": False, "email_on_retry": False, "start_date": days_ago(1), "on_failure_callback": slack_failure_msg}

with DAG(dag_id=os.path.basename(__file__).replace(".py", ""), default_args=default_args, schedule_interval=None, tags=["template", "python"]) as dag:

    gen_op_tsk = PythonOperator(task_id="gen_op_eg", python_callable=gen_op)

    read_op_tsk = PythonOperator(task_id="read_op_in_sub_task", python_callable=read_op)
